...
/obj/item/weapon/shockpaddles
	...
	var/safety = 0 //if you can zap people with the paddles on harm mode
	var/combat = 1 //If it can be used to revive people wearing thick clothing (e.g. spacesuits)
	var/cooldowntime = (5 SECONDS) // How long in deciseconds until the defib is ready again after use.
	var/chargetime = (1 SECONDS)
	var/chargecost = 0 //units of charge
...
/obj/item/weapon/shockpaddles/update_icon()
	...
	item_state = "defibpaddles"
	if(cooldown)
		icon_state = "defibpaddles_cooldown"
...
/obj/item/weapon/shockpaddles/proc/can_defib(mob/living/carbon/human/H) //This is checked before doing the defib operation
	if(H.stat == CONSCIOUS)
		return "buzzes, \"The patient is awake\""
...
/obj/item/weapon/shockpaddles/proc/do_revive(mob/living/carbon/human/H, mob/user)
	...
	if(!do_after(user, 600/user.skills[SKILL_MED], H))
		return
