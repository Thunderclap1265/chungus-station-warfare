...
/obj/item/medkit/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(M.stat == CONSCIOUS)
		playsound(get_turf(src), sound('sound/items/duct_tape.ogg',volume=100, channel=5), 50, 1)
		user.visible_message("<span class='warning'>\The [user] begins to Heal [M]</span>", "<span class='warning'>You start to heal [M]</span>")
		if(!do_after(user, 1500/user.skills[SKILL_MED], M))
