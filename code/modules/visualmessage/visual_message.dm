...
/datum/visualmessage/New(text, atom/target, mob/owner, list/extra_classes = list(), lifespan = CHAT_MESSAGE_LIFESPAN	)
	. = ..()
	if (!istype(target))
...
/datum/visualmessage/proc/generate_image(text, atom/target, mob/owner, list/extra_classes, lifespan)
...
/datum/visualmessage/proc/colorize_string(name, sat_shift = 1, lum_shift = 1)
...
/datum/visualmessage/proc/end_of_life(fadetime = CHAT_MESSAGE_EOL_FADE)
