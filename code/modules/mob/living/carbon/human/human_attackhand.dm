	...
	switch(M.a_intent)
		if(I_HELP)
			if(stat == UNCONSCIOUS)
				...
				H.visible_message("<span class='notice'>\The [H] is trying to  Help UP \the [src].</span>")

				if(!do_after(H, 2000/H.skills[SKILL_MED], src))
					return

				H.visible_message("<span class='notice'>\The [H] Helps UP \the [src]!</span>")
				revive()
