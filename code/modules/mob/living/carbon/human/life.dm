...
/mob/living/carbon/human/Life()
	...
	update_health_icon()
	spawn()
		if(stat != CONSCIOUS && !medic_message_exists&& client)
			var/medic_message
			medic_message_exists = 1
			medic_message = alert(src, "You are unconcious, if the injury was minor you might wake up on your own - otherwise wait for a medic or succumb", "What to do?", "Call a medic!", "Succumb")
			medic_message_exists = 0
			if(medic_message == "Call a medic!")
				playsound(get_turf(src), sound('sound/voice/medic_bag.ogg',volume=50, channel=2), 50, 1)
				forcesay("AAAAH I NEEED A MEDIC BAG!")
			if(medic_message == "Succumb")
				if(team == CHUNGUS_TEAM)
					chungus_tickets--
				else if(team == NOOB_TEAM)
					noob_tickets--
				ghost()
			medic_message = null
