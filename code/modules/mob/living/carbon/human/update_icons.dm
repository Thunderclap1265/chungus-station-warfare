...
#define HEALTHI_LAYER				28
#define TEAMI_LAYER				29
...
/mob/living/carbon/human/proc/update_health_icon(var/update_icons=1)
	overlays_standing[HEALTHI_LAYER] = null
	var/image/ico
	if(client == null)
		ico = overlay_image('icons/mob/health_icon.dmi',  "dead", RESET_COLOR)
	else if(src.stat == DEAD)
		ico = overlay_image('icons/mob/health_icon.dmi',  "critical", RESET_COLOR)
	else if(src.stat == UNCONSCIOUS)
		ico = overlay_image('icons/mob/health_icon.dmi',  "red", RESET_COLOR)
	else if(src.stat == CONSCIOUS && health+10 < total_health)
		ico = overlay_image('icons/mob/health_icon.dmi',  "orange", RESET_COLOR)
	else
		ico = overlay_image('icons/mob/health_icon.dmi',  "green", RESET_COLOR)
	overlays_standing[HEALTHI_LAYER] = ico
	if(update_icons)   update_icons()



/mob/living/carbon/human/proc/update_team_icon(var/update_icons=1)
	overlays_standing[TEAMI_LAYER] = null
	var/image/ico = null
	if(team == NOOB_TEAM)
		ico = overlay_image('icons/mob/team_icon.dmi',  "noob", RESET_COLOR)
	else if(team == CHUNGUS_TEAM)
		ico = overlay_image('icons/mob/team_icon.dmi',  "chungus", RESET_COLOR)
	overlays_standing[TEAMI_LAYER] = ico
	if(update_icons)   update_icons()
